...
nstry = 1  # number of stories

## --- section_ma: (GMTrussStructure) setting list of nodes --- ##
nodes = list(range(2 + 2 * nstry))
leng, exfc = 1., 100.  # (m), (kN)
if nstry == 1:
    nodes[0] = GMTrussNode(xxyy=(leng*0, leng*0), fixc=(True, True), locn=(0, 1))
    nodes[1] = GMTrussNode(xxyy=(leng*1, leng*0), fixc=(True, True), locn=(2, 3))
    nodes[2] = GMTrussNode(xxyy=(leng*0, leng*1), fixc=(False, False), locn=(4, 5))
    nodes[2]._exfc.set_vector(xxyy=(exfc, 0.))
    nodes[3] = GMTrussNode(xxyy=(leng*1, leng*1), fixc=(False, False), locn=(6, 7))
if nstry == 2:
    nodes[0] = GMTrussNode(xxyy=(leng*0, leng*0), fixc=(True, True), locn=(0, 1))
    nodes[1] = GMTrussNode(xxyy=(leng*1, leng*0), fixc=(True, True), locn=(2, 3))
    nodes[2] = GMTrussNode(xxyy=(leng*0, leng*1), fixc=(False, False), locn=(4, 5))
    nodes[3] = GMTrussNode(xxyy=(leng*1, leng*1), fixc=(False, False), locn=(6, 7))
    nodes[4] = GMTrussNode(xxyy=(leng*0, leng*2), fixc=(False, False), locn=(8, 9))
    nodes[4]._exfc.set_vector(xxyy=(exfc, 0.))
    nodes[5] = GMTrussNode(xxyy=(leng*1, leng*2), fixc=(False, False), locn=(10, 11))
...
if nstry == 1:
    ...
    membs[2] = GMTrussMember(nodea=nodes[2], nodeb=nodes[3], area=area, yong=yong)
    membs[3] = GMTrussMember(nodea=nodes[0], nodeb=nodes[3], area=area, yong=yong)
    membs[4] = GMTrussMember(nodea=nodes[1], nodeb=nodes[2], area=area, yong=yong)
if nstry == 2:
    membs[0] = GMTrussMember(nodea=nodes[0], nodeb=nodes[2], area=area, yong=yong)
    membs[1] = GMTrussMember(nodea=nodes[1], nodeb=nodes[3], area=area, yong=yong)
    membs[2] = GMTrussMember(nodea=nodes[2], nodeb=nodes[3], area=area, yong=yong)
    ...
    membs[7] = GMTrussMember(nodea=nodes[4], nodeb=nodes[5], area=area, yong=yong)
    membs[8] = GMTrussMember(nodea=nodes[2], nodeb=nodes[5], area=area, yong=yong)
    membs[9] = GMTrussMember(nodea=nodes[3], nodeb=nodes[4], area=area, yong=yong)
...
strc.graph(
    ...
    xlim=(-1.0,1+nstry), ylim=(-1.0, 1+nstry), fname='_a1')
