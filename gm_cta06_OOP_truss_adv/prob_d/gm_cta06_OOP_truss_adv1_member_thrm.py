# gm_cta06_OOP_truss_adv1_member_wght.py: coded by Kinya MIURA 240530
# ---------------------------------------------------------
...
import copy
from gm_cta06_OOP_truss_adv0_node_thrm import (GMTrussNode)
...
class GMTrussMember():
    ...
    def __init__(self,
            ...
            area: float = 10., yong: float = 205.,
            rhos: float = 0., grac: float = 9.80665,
            temp: float = 0., alph: float = 11.e-6, cnv: bool = True ):
        ...
        self.__rhos, self.__grac = 0., 0.
        # density of steal, gravity acceleration
        self.__temp, self.__alph = 0., 0.
        # temperature, thermal expansion coefficient
        self.set_truss_member(area=area, yong=yong, rhos=rhos, grac=grac, temp=temp, alph=alph, cnv=cnv )
    ...
    def set_truss_member(self,
            ...
            rhos: float = None, grac: float = None,
            temp: float = None, alph: float = None,
            cnv: bool = True ) -> None:
        if area is not None:
        ...
        if rhos is not None:
            self.__rhos = rhos  # (kg/m^3)
        if grac is not None:
            self.__grac = grac  # (m/s^2)
        if temp is not None:
            self.__temp = temp  # (K)
        if alph is not None:
            self.__alph = alph  # (/K)
    ...
    def rhos(self, cnv: bool = True) -> float:
        return self.__rhos  # (kg/m^3)
    def grac(self, cnv: bool = True) -> float:
        return self.__grac  # (m/s^2)
    def temp(self, cnv: bool = True) -> float:
        return self.__temp  # (K)
    def alph(self, cnv: bool = True) -> float:
        return self.__alph  # (/K)
    ...
    def __str__(self) -> str:
        st = (
            ...
            f'rhos (kg/m^3) = {self.rhos():g}, grac (m/s^2) = {self.grac():g}, '
            f'temp (K) = {self.temp():g}, alph (/K) = {self.alph():g} ' )
        return st
    ...
    def wght(self, cnv: bool = True) -> float:
        wght = self.__rhos * self.__grac * self.__area * self.leng(False)
        return wght / 1e+3 if cnv else wght  # (kN)
    def thrm(self, cnv: bool = True) -> float:
        thrm = self.__temp * self.__alph * self.__area * self.yong(False)
        return thrm / 1e+3 if cnv else thrm  # (kN)
    def calc_stiffness(self) -> ndarray:  # building stiffness matrix
        ...
        return stif
    def calc_weight(self) -> None:  # calculating stiffness matrix
        wght = self.wght(False)
        self._nodea._wght.add((0,-wght/2))
        self._nodeb._wght.add((0,-wght/2))
    def calc_thermal(self) -> None:  # calculating stiffness matrix
        thrm = self.thrm(False)
        self._nodea._thrm.add(thrm*self.unitvect_na())
        self._nodeb._thrm.add(thrm*self.unitvect_nb())
...
if __name__ == '__main__':
    ...
    # ---------------------------------------------------------
    memb_0 = GMTrussMember(nodea=node_0, nodeb=node_1, rhos=0, temp=20)
    memb_1 = GMTrussMember(nodea=node_1, nodeb=node_2, rhos=0, temp=20)
