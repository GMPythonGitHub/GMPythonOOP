...
from numpy import (
    array, append, zeros as zers, full, ix_, inner, logical_not as lognot)
from numpy.linalg import solve
from gm_cta06_OOP_truss_adv1_member_thrm import (GMTrussNode, GMTrussMember)
...
class GMTrussStructure():
    ...
    def __init__(self, nodes: list, membs: list):
        ...
        self._exfc, self._wght, self._thrm = array([]), array([]), array([])
    ...
    def set_truss_structure(self, nodes: list, membs: list) -> None:
        ...
        self._wght, self._thrm = zers((degfred,)), zers((degfred,))
    ...
    def solv_matrixeq(self) -> None:  # solving matrix equation
        ...
        bb = self._exfc[ix_(frec)] - inner(self._stif[ix_(frec, fixc)], self._disp[ix_(fixc)])
        bb += self._wght[ix_(frec)] + self._thrm[ix_(frec)]
        xx = solve(aa,bb)  # solving matrix equation
        self._disp[ix_(frec)] = xx
        self._exfc = inner(self._stif, self._disp) - self._wght - self._thrm
...
if __name__ == '__main__':
    ...
    membs[0] = GMTrussMember(
        nodea=nodes[0], nodeb=nodes[2], area=area, yong=yong, temp=temp, alph=alph)
    membs[1] = GMTrussMember(
        nodea=nodes[1], nodeb=nodes[3], area=area, yong=yong, temp=temp, alph=alph)
    membs[2] = GMTrussMember(
        nodea=nodes[2], nodeb=nodes[3], area=area, yong=yong, temp=temp, alph=alph)
    membs[3] = GMTrussMember(
        nodea=nodes[0], nodeb=nodes[3], area=area, yong=yong, temp=temp, alph=alph)
    ...
    strc.graph(
        scl_dfm=0.05, scl_frc=0.005, xlim=(-1.0,2.0), ylim=(-1.0, 2.0) )
