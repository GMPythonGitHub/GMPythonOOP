# gm_cta06_OOP_truss_adv_prob_c1.py: coded by Kinya MIURA 240530
# ---------------------------------------------------------
...
## --- section_mc: (GMTrussStructure) setting self wight --- ##")
rho, gac = 7900, 9.80665
wght = None
for memb in membs:
    wght = rho * gac * memb.area(False) * memb.leng(False)
    memb._nodea._exfc.add((0,-wght/2))
    memb._nodeb._exfc.add((0,-wght/2))
print(f'{wght = }')
...
strc.graph(
    ...
    xlim=(-1,1+nstry), ylim=(-2, 1+nstry-1), fname='_c1' )
