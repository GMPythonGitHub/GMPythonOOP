# gm_cta06_OOP_truss_adv_prob_c2_.py: coded by Kinya MIURA 240530
# ---------------------------------------------------------
...
from gm_cta06_OOP_truss_adv2_structure_wght import (
...
for istry in range(nstry):  # n-story membs
    ...
    membs[membo+0] = GMTrussMember(
        nodea=nodes[nodeo+0], nodeb=nodes[nodeo+2], area=area, yong=yong, rhos=rhos, grac=grac)
    membs[membo+1] = GMTrussMember(
        nodea=nodes[nodeo+1], nodeb=nodes[nodeo+3], area=area, yong=yong, rhos=rhos, grac=grac)
    membs[membo+2] = GMTrussMember(
        nodea=nodes[nodeo+2], nodeb=nodes[nodeo+3], area=area, yong=yong, rhos=rhos, grac=grac)
    membs[membo+3] = GMTrussMember(
        nodea=nodes[nodeo+0], nodeb=nodes[nodeo+3], area=area, yong=yong, rhos=rhos, grac=grac)
...
strc.graph(
    scl_dfm=0.05/nstry*100, scl_frc=0.005/nstry*100,
    xlim=(-1,1+nstry), ylim=(-2, 1+nstry-1), fname='_c2' )
